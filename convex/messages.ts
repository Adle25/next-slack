...
const populateThread = async (ctx: QueryCtx, messageId: Id<"messages">) => {
    const messages = await ctx.db.query("messages").withIndex("by_parent_message_id", (q) => q.eq("parrentMessageId", messageId)).collect();
...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await ctx.db.query("messages").withIndex("by_channel_id_parent_message_id_conversation_id", (q) => q.eq("channelId", args.channelId).eq("parrentMessageId", args.parentMessageId).eq("conversationId", _conversationId)).order("desc").paginate(args.paginationOpts);
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const messageId = await ctx.db.insert("messages", {
            ...
            parrentMessageId: args.parentMessageId,
        })
